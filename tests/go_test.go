...
const COMMAND = "../target/debug/duckrg"
...
func mkRaw(arr []interface{}) []json.RawMessage {
...
// CHANGE queries can be run as statements
/*
	...
	req := request{
		Transaction: []requestItem{
			...
			},
			{
				...
				Values:    mkRaw([]interface{}{2, "TWO"}),
			},
			{
				Statement: "INSERT INTO T1 (ID, VAL) VALUES (?, ?)",
				ValuesBatch: [][]json.RawMessage{
					mkRaw([]interface{}{3, "THREE"}),
					mkRaw([]interface{}{4, "FOUR"})},
			},
			{
				Query:  "SELECT * FROM T1 WHERE ID > ?",
				Values: mkRaw([]interface{}{0}),
			},
		},
	}
...
func TestConcurrent(t *testing.T) {
	...
	for i := 0; i < concurrency; i++ {
		go func(t *testing.T) {
			...
			require.Equal(t, "ONE", res.Results[1].ResultSet[0]["VAL"])
			...
			require.Equal(t, 4, len(res.Results[4].ResultSet))
		}(t)
	}
	wg.Wait()
}
...
func TestInitBackup(t *testing.T) {
	...
	require.DirExists(t, fmt.Sprintf("env/backups/test_%s", now()))
}
...
func TestPeriodicBackupWith1File(t *testing.T) {
	...
	bkp1 := fmt.Sprintf("env/backups/test_%s", now())
	// require.FileExists(t, bkp1)

	time.Sleep(60 * time.Second)
	bkp2 := fmt.Sprintf("env/backups/test_%s", now())
	require.NoDirExists(t, bkp1)
	require.DirExists(t, bkp2)
}
...
func TestFailRO(t *testing.T) {
	...
	defer setupTest(t, &cfg, true, "--mem-db", "test")(true)
	...
	code, body, _ := call(t, "http://localhost:12321/test", req)
...
func TestProfilerPayloadOnFile(t *testing.T) {
	...
	require.Equal(t, 1, len(res.Results[4].RowsUpdatedBatch))
	require.Equal(t, 1, res.Results[4].RowsUpdatedBatch[0])
	require.Equal(t, 1, len(res.Results[5].ResultSet))
	require.Equal(t, 4, *res.Results[6].RowsUpdated)
...
func TestAuthOkHttpPlaintextByQuery(t *testing.T) {
	cfg := db{
		Auth: &authr{
			Mode:    "HTTP_BASIC",
			ByQuery: "SELECT 1 FROM AUTH WHERE USER = ? AND PASS = ?",
		},
		Macros: []macro{
			{
				Id: "M1",
...
func TestBothValueAndBatchFail(t *testing.T) {
	...
	req := request{
		Transaction: []requestItem{
			{
				Statement: "INSERT INTO TBL (ID, VAL) VALUES (?, ?)",
				Values:    mkRaw([]interface{}{0, "zero"}),
				ValuesBatch: [][]json.RawMessage{
					mkRaw([]interface{}{1, "uno"}),
					mkRaw([]interface{}{2, "due"}),
				},
			},
		},
	}
...
func TestReturnedString(t *testing.T) {
	...
	req := request{
		Transaction: []requestItem{
			{
				Statement: "INSERT INTO TBL (ID, VAL) VALUES (1, ?)",
				Values:    mkRaw([]interface{}{ciao}),
			}, {
...
func TestReturnedBigInteger(t *testing.T) {
	...
	req := request{
		Transaction: []requestItem{
			{
				Statement: "INSERT INTO TBL VALUES(?)",
				Values:    mkRaw([]interface{}{test}),
			},
			{
		...
		Macros: []macro{
			{
				Id: "M1",
				Statements: []string{
					"CREATE TABLE IF NOT EXISTS TBL (VAL DOUBLE)",
				},
				Execution: execution{
					OnCreate: &TRUE,
				},
...
func TestReturnedBool(t *testing.T) {
		...
		Macros: []macro{
			{
				Id: "M1",
				Statements: []string{
					"CREATE TABLE IF NOT EXISTS TBL (VAL BOOLEAN)",
				},
				Execution: execution{
					OnCreate: &TRUE,
				},
	...
	require.True(t, ret.Results[1].ResultSet[0]["VAL"].(bool))
}
