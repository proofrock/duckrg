...
pub fn delete_old_bkp_dirs(dir: &str, files_to_keep: usize) -> Result<()> {
...
pub fn if_abort_duckdb<T>(result: duckdb::Result<T, duckdb::Error>) -> T {
    result.unwrap_or_else(|e| abort(e.to_string()))
}
...
// Utils to convert serde structs to slices accepted by duckdb as named params
// adapted from serde-rusqlite, https://github.com/twistedfall/serde_rusqlite/blob/master/LICENSE

pub struct ParamsContainer(Vec<Box<dyn duckdb::types::ToSql>>);

impl ParamsContainer {
    pub fn slice(&self) -> Vec<&dyn duckdb::types::ToSql> {
        self.0.iter().map(|el| (el.borrow())).collect()
    }
}

impl From<Vec<Box<dyn duckdb::types::ToSql>>> for ParamsContainer {
    fn from(src: Vec<Box<dyn duckdb::types::ToSql>>) -> Self {
        Self(src)
    }
}
