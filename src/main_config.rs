...
use crate::commons::{
    abort, assert, file_exists, if_abort_duckdb, is_dir, is_file_in_directory, resolve_tilde,
...
fn compose_single_db(
    ...
    let config = if dbconf.read_only {
        Config::default().access_mode(AccessMode::ReadOnly).unwrap()
    } else {
        Config::default()
    };

    let semi_conn = if is_mem {
        // an in-memory db cannot be open as read-only, duckdb dutifully returns an error
        Connection::open_in_memory_with_flags(config)
    } else {
        Connection::open_with_flags(conn_string, config)
    };

    let mut conn = if_abort_duckdb(semi_conn);
